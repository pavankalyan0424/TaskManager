...
def user_interface(peer_list, peer_list_lock, my_addr, auth_token):
    prompt = (
        "\nEnter task (word_count, file_count, sha256, line_count,"
        " file_metadata, show_peers, help, exit): "
    )
    while True:
        try:
            ...
            if cmd == "":
                # Re-prompt on empty input or line breaks
                continue
            ...
            if cmd == "file_metadata":
                ...
                def send_file_processing_task_to_peer(p, fpath):
                    try:
                        logger.info(
                        ...
                        res = {"status": "error", "message": str(exception)}
                    with results_lock:
                        results[(p, fpath)] = res

                # Logic: if more than one file is passed,
                # then files are distributed among all the peers
                if len(file_list) == 1:
                    chosen_peer = random.choice(targets)
                    send_file_processing_task_to_peer(chosen_peer, file_list[0])
                else:
                    for i, fp in enumerate(file_list):
                        peer = targets[i % len(targets)]
                        t = threading.Thread(
                            target=send_file_processing_task_to_peer,
                            args=(peer, fp),
                            daemon=True,
                        )
                        threads.append(t)
                ...
                for (peer, file_path), resp in results.items():
                    if resp and resp.get("status") == "success":
                        print(f"\n[REPLY] from {peer} for '{file_path}':")
                        for key, value in resp["result"].items():
                            print(f"  {key}: {value}")
                    else:
                        print(
                            f"[ERR] from {peer} for '{file_path}': {resp.get('message') if resp else 'No response'}"
                        )
            ...
            if resp and resp.get("status") == "success":
                logger.info(f"[RESULT] from {target}: {resp.get('result')}")
            else:
                logger.info(
