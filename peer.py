...
def gossip_with_peers():
    while True:
        try:
            ...
            # gossip with a random peer (exchange known peers) silently
            with peer_list_lock:
                peers_snapshot = list(peer_list)
            if peers_snapshot:
            ...
            # silent on gossip thread errors
            pass
        time.sleep(10)
...
def handle_rpc_request(conn, addr):
    try:
        ...
        if task not in ("ping", "gossip"):
            logger.info(f"[PROC] Received task '{task}' from peer {sender}")
...
def peer_server():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            conn, addr = s.accept()
            threading.Thread(
                target=handle_rpc_request, args=(conn, addr), daemon=True
            ).start()
...
def user_interface():
    ...
    while True:
        ...
        if cmd == "file_metadata":
            ...
            with peer_list_lock:
                # include self as a potential worker
            ...
            if len(file_list) == 1:
                ...
                if chosen_peer == MY_ADDR:
                    ...
                    resp = send_rpc_call(
                        chosen_peer[0],
                        chosen_peer[1],
                        {"task": "file_metadata", "file_path": file_list[0]},
                    )
                    results[(chosen_peer, file_list[0])] = resp
            else:
                # Multiple files -> distribute round-robin across targets (including self)
                print(
                    f"[SEND] Distributing {len(file_list)} files among {len(targets)} peers..."
                )
                ...
                def query_peer(p, fpath):
                    try:
                        ...
                        if p == MY_ADDR:
                            res = {
                                "status": "success",
                                "result": get_file_metadata(fpath),
                            }
                        else:
                            res = send_rpc_call(
                                p[0],
            ...
            for (peer, fpath), resp in results.items():
                if resp and resp.get("status") == "success":
                    ...
                    print(
                        f"[ERR] from {peer} for '{fpath}': {resp.get('message') if resp else resp}"
                    )
            continue
